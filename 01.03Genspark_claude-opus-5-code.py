...
# 🎨 ألوان ومساعدات
# ══════════════════════════════════════════════════════════════
# ملاحظة تدقيق (Audit 2026-08-23): أُزيل تعريف _DIR المكرر هنا —
# التعريف الوحيد المعتمد أعلاه (يُستخدم في Config قبل هذه النقطة).
...
# ══════════════════════════════════════════════════════════════
# 💰 فحص الرصيد واشتراك الحساب من الـ API الحقيقي للمتصفح
# ══════════════════════════════════════════════════════════════
def get_subscription_info(cookies: dict) -> dict:
    """استعلام مباشر عن الرصيد الحقيقي ونوع الاشتراك من الـ APIs الرسمية للمتصفح"""
    out = {"plan": "free", "sub_status": "none", "credit_balance": None}
    if not cookies:
        return out
    try:
        from curl_cffi import requests as cffi
        ...
        r_bal = cffi.get("https://www.genspark.ai/api/payment/get_credit_balance", cookies=cookies, headers=headers, timeout=10)
        if r_bal.status_code == 200:
            d = r_bal.json().get("data", {})
            if "balance" in d and d["balance"] is not None:
                out["credit_balance"] = int(d["balance"])
        elif r_bal.status_code == 401:
            out["credit_balance"] = -2
    except Exception:
        pass
    try:
        from curl_cffi import requests as cffi
        ...
        r_sub = cffi.get("https://www.genspark.ai/api/payment/current_subscriptions", cookies=cookies, headers=headers, timeout=10)
        if r_sub.status_code == 200:
            data = r_sub.json()
            out["plan"] = data.get("membershipType") or data.get("subType") or "free"
            out["sub_status"] = data.get("membershipStatus") or data.get("subStatus") or "active"
    except Exception:
        pass
    return out
...
def check_balance(cookies: dict) -> int:
    """يسأل API عن الرصيد الحقيقي — يرجع int أو -1 لو فشل أو -2 لو جلسة منتهية"""
    info = get_subscription_info(cookies)
    if info.get("credit_balance") is not None:
        return info["credit_balance"]
    try:
        from curl_cffi import requests as cffi
        sess = cffi.Session(impersonate="chrome120")
...
# ══════════════════════════════════════════════════════════════
def _print_public_share_link(public_url: str) -> None:
...
def _do_auto_share(cfg: Config, conv_name: str, project_id: str, cookies: dict) -> str | None:
    """
    شير ذكي موحّد — [R1] دمج النسختين (كانت الثانية تحجب الأولى بحكم بايثون):
    - يحترم مفتاح cfg.auto_share (كان مُعطّلاً بالحجب — الشير كان يعمل حتى لو مُطفأ)
    - كاش: لو موجود public_url محفوظ لنفس الـ project → برجع من غير API call (وفّر)
    - لو مش موجود → يجرب share_retry مرة مع retry
    - يطبع الرابط العام بلون بينك نيون فاقع + fallback لرابط الـ viewer عند فشل الشير
      (ميزتا النسخة الثانية محفوظتان)
    """
    if not cfg.auto_share or not project_id or not cookies:
        return None
    ...
    if saved_url and project_id in saved_url:
        _print_public_share_link(saved_url)
        return saved_url  # نفس الـ project → متشارك فعلاً (بدون API call)
    ...
    for attempt in range(max(1, cfg.share_retry)):
        ...
        if url:
            ...
            _print_public_share_link(url)
            return url
        if attempt < cfg.share_retry - 1:
            time.sleep(2)  # انتظر ثانيتين وجرّب تاني
    # فشل الشير → fallback لرابط الـ viewer العام (سلوك النسخة الثانية محفوظ)
    if cfg.show_debug:
        p(Fore.RED, "  ❌ الشير فشل — استخدام رابط الـ viewer كبديل")
    fallback_url = f"{GENSPARK}/autopilotagent_viewer?id={project_id}"
    _print_public_share_link(fallback_url)
    return fallback_url
...
def filter_messages_after_compact(messages: list) -> list:
    """
    مطابقة 100% لكود Nuxt (B73RK9LQ.js L422-L430):
    let U = -1;
    for (let oe = w.length - 1; oe >= 0; oe--) {
        const qe = w[oe];
        if (qe.session_state && qe.session_state.is_compact_summary === !0) {
            U = oe;
            break;
        }
    }
    ...
    قص وحذف الرسائل القديمة والبدء فقط من رسالة التلخيص الأحدث (is_compact_summary: true).
    """
    if not messages:
        return []
    compact_idx = -1
    for i, msg in enumerate(messages):
        if not isinstance(msg, dict):
            continue
        session_state = msg.get("session_state") or {}
        if isinstance(session_state, dict) and session_state.get("is_compact_summary") is True:
            compact_idx = i
            
    if compact_idx != -1:
        return messages[compact_idx:]
...
def fetch_project_messages(project_id: str, cookies: dict, cfg: "Config" = None) -> list:
    ...
    مطابق للمتصفح في ملفات الـ HAR: يمرر كائنات الرسائل كاملة بجميع حقولها (25 مفتاحاً)
    بما فيها tool_calls و session_state التي تحتوي على is_compact_summary و chat_session_id
    """
    if not project_id:
        return []
    if cfg is not None:
    ...
    try:
        ...
        # 1. الاستدعاء المباشر النقي لـ /api/project?id= (مطابق للمتصفح 100% في ملفات الـ HAR)
        api_url = f"{GENSPARK}/api/project?id={project_id}"
        r = sess.get(api_url, timeout=20)
        if cfg is not None:
            setattr(cfg, "_last_fetch_status", r.status_code)   # B1: المستدعي يفرّق 404 عن "فاضي"
        if r.status_code == 200:
            try:
                res_json = r.json()
                ...
                # 🔍 الدليل القاطع من ملفات الـ HAR:
                # المتصفح لا يقوم بتجريد أي مفتاح من الرسائل، بل يمرر كائن الرسالة كاملاً بـ 25 حقلاً
                valid_msgs = [m for m in msgs if isinstance(m, dict) and "role" in m]
                if valid_msgs:
                    orig_count = len(valid_msgs)
                    valid_msgs = filter_messages_after_compact(valid_msgs)
                    if cfg and getattr(cfg, "show_debug", False):
                        has_comp = any((m.get("session_state") or {}).get("is_compact_summary") for m in valid_msgs)
                        p(Fore.CYAN, f"  📥 جاب {len(valid_msgs)}/{orig_count} رسالة بعد فلترة الكومباكت من /api/project (Compacted={has_comp} | SessionID={s_chat_id[:8] if s_chat_id else 'None'})")
                    return valid_msgs
            except Exception:
                pass

        # 2. Fallback: لو الـ API المباشر ملقاش بيانات، نلجأ للـ Robust Nuxt Resolver
        r_page = sess.get(f"{GENSPARK}/agents?id={project_id}", timeout=25)
        if r_page.status_code == 200:
            match = re.search(r'<script[^>]*id="__NUXT_DATA__"[^>]*>(.*?)</script>', r_page.text, re.DOTALL)
            if match:
                raw = json.loads(match.group(1))
                if isinstance(raw, list):
                    def resolve_val(idx, depth=0):
                        if depth > 5 or not isinstance(idx, int) or idx < 0 or idx >= len(raw):
                            return idx
                        val = raw[idx]
                        if isinstance(val, list) and len(val) == 2 and val[0] in ("ShallowReactive", "Reactive"):
                            return resolve_val(val[1], depth + 1)
                        return val

                    messages = []
                    for item in raw:
                        if isinstance(item, dict) and "role" in item and "content" in item:
                            r_role = resolve_val(item.get("role"))
                            r_content = resolve_val(item.get("content"))
                            if isinstance(r_role, str) and isinstance(r_content, str):
                                messages.append({
                                    "role": r_role,
                                    "content": r_content,
                                    "id": str(resolve_val(item.get("id")) or ""),
                                })
                    if messages:
                        orig_count = len(messages)
                        messages = filter_messages_after_compact(messages)
                        if cfg and getattr(cfg, "show_debug", False):
                            p(Fore.CYAN, f"  📥 جاب {len(messages)}/{orig_count} رسالة عبر Nuxt Fallback بعد فلترة الكومباكت")
                        return messages
    except Exception as e:
        if cfg and getattr(cfg, "show_debug", False):
            p(Fore.YELLOW, f"  ⚠️ fetch_project_messages error: {e}")
    return []
...
server_continue_conversation = None # set below
...
server_continue_conversation = create_forked_project
...
def apply_model_contract(payload: dict, model: str | None, msg_id: str | None = None) -> dict:
    ...
    # 2. العقود المعتمدة
    if m in ("claude-fable-5-1", "claude-fable-5.1", "claude fable 5.1", "fable 5.1", "claude-fable-5", "claude fable 5", "fable 5"):
        payload["models"] = ["gpt-4.1"]
        payload["use_model"] = "claude-fable-5-1"
        if msg_id:
            payload["client_message_id"] = msg_id
    elif m in ("gpt-6-astra", "gpt 6 astra", "astra"):
        payload["models"] = ["gpt-4.1"]
        payload["use_model"] = "gpt-6-astra"
        if msg_id:
            payload["client_message_id"] = msg_id
        ...
        payload["models"] = [model] if model else ["claude-fable-5-1"]
...
def send_chat(
    ...
    # ── تقليص الـ History توفيراً للرصيد (Tokens) — الحفاظ على الذاكرة دون تصفير ──
    ...
    if cfg.model in ("gpt-5.5", "claude-opus-4-8"):
        ...
        if _is_continue:
            ...
            if (fork_project_id and not project_id and not old_msgs
                ...
                p(Fore.YELLOW, f"  ♻️ مصدر الـ Fork مش موجود ({cfg._last_fetch_status}) → __INVALID_PROJECT__")
                return None, "__INVALID_PROJECT__", None
            s_chat_id = getattr(cfg, "_last_chat_session_id", None)
            if s_chat_id:
                payload["chat_session_id"] = s_chat_id

            if old_msgs:
                compact_idx = -1
                for idx, m in enumerate(old_msgs):
                    m_ss = m.get("session_state") or {}
                    if m_ss.get("is_compact_summary"):
                        compact_idx = idx
                        break

                if compact_idx != -1:
                    compact_msg = old_msgs[compact_idx]
                    post_compact_msgs = old_msgs[compact_idx + 1:]
                    selected_history = [compact_msg] + post_compact_msgs
                    if getattr(cfg, "show_debug", False):
                        p(Fore.GREEN, f"  🌐 [5.5 Pro] دمج Compact Summary + {len(post_compact_msgs)} رسائل لاحقة")
                else:
                    selected_history = old_msgs

                context_messages = selected_history + [new_msg]
                ...
                if project_id:
                    payload["force"] = True
                p(Fore.CYAN, f"  🧠 [5.5 Pro] Dynamic Browser Parity: {len(selected_history)} رسالة قديمة + رسالتك")
        elif history:
        ...
        payload = {
            "models":                  ["gpt-4.1"] if cfg.model in ("claude-fable-5-1", "gpt-6-astra") else [cfg.model],
            "run_with_another_model":  False,
            "request_web_knowledge":   cfg.request_web_knowledge,
            "speed_mode":              False if _is_continue else True,                 # مطابق لملفات الـ HAR: في الاستكمال دايماً False
        ...
        # ── Continue: Fetch & Forward (Dynamic Browser Parity) ──
        if _is_continue:
            if project_id:
                payload["force"] = True
            ...
            if old_msgs:
                # 1. فحص وجود رسالة الـ Compact Summary (رقم 0) داخل session_state كما في الـ HAR
                compact_idx = -1
                for idx, m in enumerate(old_msgs):
                    m_ss = m.get("session_state") or {}
                    if m_ss.get("is_compact_summary"):
                ...
                if compact_idx != -1:
                    # ✅ مطابقة المتصفح في ملفات HAR (Opus_5..3 بـ 74 رسالة و Opus_5..4 بـ 11 رسالة):
                    # نرسل كبسولة الـ Summary رقم 0 + كل الرسائل التي تلتها بالكامل بدون أي رقم ثابت
                    compact_msg = old_msgs[compact_idx]
                    post_compact_msgs = old_msgs[compact_idx + 1:]
                    selected_history = [compact_msg] + post_compact_msgs
                    if getattr(cfg, "show_debug", False):
                        p(Fore.GREEN, f"  🌐 [Browser Parity] تم دمج Compact Summary + {len(post_compact_msgs)} رسائل لاحقة (إجمالي {len(selected_history)})")
                else:
                    selected_history = old_msgs
                ...
                payload["messages"] = context_messages
                p(Fore.CYAN, f"  🧠 Fetch & Forward (Browser Parity): {len(selected_history)} رسالة قديمة + رسالتك")
            else:
    ...
    try:
                ...
                try:
                    ...
                    if t == "message_start" and obj.get("id"):
                        ...
                        if active_msg_id not in messages_by_id:
                            ...
                            message_order.append(active_msg_id)

                    # streaming delta — تجميع صامت داخل الرسالة النشطة لعزل الأدوات
                    if t == "message_field_delta" and obj.get("field_name") == "content":
                        chunk = obj.get("delta", "")
                        mid = obj.get("message_id") or active_msg_id
                        if mid:
                            ...
                            messages_by_id[mid]["content"] += chunk
                        full_text += chunk
                    ...
                    if t == "message_field" and obj.get("field_name") == "content":
                        ...
                        if mid and fv:
                            if mid not in messages_by_id:
                                ...
                                message_order.append(mid)
                            messages_by_id[mid]["content"] = fv

                    # نتيجة كاملة (مطابقة 100% لحزم الـ HAR الحقيقية)
                    if t == "message_result":
        ...
        # 🎯 استخلاص الرد النهائي الصافي من بين الرسائل المتعددة (عزل الأدوات والتفكير والـ Terminal):
        if not is_credit_exhausted:
            clean_candidates = []
            for mid in reversed(message_order):
                m = messages_by_id[mid]
                if m.get("is_credit_out"):
                    continue
                m_role = m.get("role")
                m_content = (m.get("content") or "").strip()
                m_tools = m.get("tool_calls")
                m_finish = m.get("finish_reason")

                # الرسالة الصافية للمستخدم: assistant وبدون استدعاء أدوات
                if m_role == "assistant" and m_content and not m_tools:
                    clean_candidates.append((mid, m_content, m_finish))

            if clean_candidates:
                # نفضل الرسالة ذات finish_reason == 'stop' أو آخر رسالة مستوفية
                best_mid, best_content, _ = clean_candidates[0]
                for cmid, ccontent, cfinish in clean_candidates:
                    if cfinish == "stop":
                        best_mid, best_content = cmid, ccontent
                        break
                full_text = best_content
                asst_msg_id = best_mid
            elif not full_text:
                # Fallback: لو لم تسجل رسائل مستوفية، نأخذ آخر محتوى نصي غير فارغ
                for mid in reversed(message_order):
                    cnt = (messages_by_id[mid].get("content") or "").strip()
                    if cnt:
                        full_text = cnt
                        asst_msg_id = mid
                        break
...
# [R1] أُزيل التعريف الثاني المكرر لـ _do_auto_share من هنا — كان يحجب النسخة
# الذكية (كاش + retry + احترام cfg.auto_share) ويسبب API call مع كل رسالة.
# ميزاته (الطباعة البينك + fallback للـ viewer) دُمجت في النسخة الموحّدة أعلاه.
...
def get_real_balance(cookies: dict) -> int:
    ...
    except Exception:  # [R2] كان bare except يبتلع KeyboardInterrupt/SystemExit
        pass
    return 0
