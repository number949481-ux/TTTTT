...
DEFAULT_PROJECT_MODEL = "claude-fable-5-1"
...
MODEL_DISPLAY_NAMES = {
    ...
    "gpt-5.6-sol": "☀️ GPT-5.6 Sol",
    "kimi-k3": "🚀 Kimi K3",
}

MODEL_ALIASES = {
    "claude-fable-5-1": "claude-fable-5-1", "claude-fable-5.1": "claude-fable-5-1", "claude fable 5.1": "claude-fable-5-1", "claude-fable-5": "claude-fable-5-1", "claude fable 5": "claude-fable-5-1",
    "fable 5.1": "claude-fable-5-1", "fable 5": "claude-fable-5-1", "🧠 claude fable 5.1": "claude-fable-5-1",
    "claude-opus-5": "claude-opus-5", "claude opus 5": "claude-opus-5", "opus 5": "claude-opus-5", "👑 claude opus 5": "claude-opus-5",
    "claude-sonnet-5": "claude-sonnet-5", "claude sonnet 5": "claude-sonnet-5", "sonnet 5": "claude-sonnet-5", "🎭 claude sonnet 5": "claude-sonnet-5",
    "gpt-6-astra": "gpt-6-astra", "gpt-6 astra": "gpt-6-astra", "gpt 6 astra": "gpt-6-astra", "astra": "gpt-6-astra", "⚡ gpt-6 astra": "gpt-6-astra", "⚡ gpt-6-astra": "gpt-6-astra",
    "gpt-5.6-sol": "gpt-5.6-sol", "gpt 5.6 sol": "gpt-5.6-sol", "gpt 5.6": "gpt-5.6-sol", "☀️ gpt-5.6 sol": "gpt-5.6-sol",
    "kimi-k3": "kimi-k3", "kimi k3": "kimi-k3", "k3": "kimi-k3", "🚀 kimi k3": "kimi-k3",
    "gpt-5.5": "gpt-5.5", "gpt 5.5": "gpt-5.5", "gpt5.5": "gpt-5.5",
    "claude-opus-4-8": "claude-opus-4-8", "claude opus 4.8": "claude-opus-4-8", "claude opus 4.8 pro": "claude-opus-4-8",
}

# ── عقود الموديلات (CONTRACTS) — SSOT موحد ──────────────────────────────
# model_slug → (models_list, use_model, ai_chat_model, inject_msg_id)
CONTRACTS = {
    "claude-fable-5-1":  (["gpt-4.1"],          "claude-fable-5-1",  None,              True),
    "claude-opus-5":   (["claude-opus-5"],    "claude-opus-5",   None,              True),
    "claude-sonnet-5": (["claude-sonnet-5"],  "claude-sonnet-5", "claude-sonnet-5", True),
    "gpt-6-astra":     (["gpt-4.1"],          "gpt-6-astra",     None,              True),
...
def format_model_display_label(model: str | None) -> str:
    """إرجاع اسم الموديل المنسق لعين المستخدم في واجهة تيليجرام (كابتل + إيموجي)"""
    canonical = normalize_project_model(model)
    return MODEL_DISPLAY_NAMES.get(canonical, canonical or str(model or ""))

DEFAULT_PROJECT_RESUME_PROMPT = "تابع"
PROJECT_SECRET_SCHEMA_VERSION = 1

# [P41] رسالة الرفض الصريحة للرابط المشوّه (بلا Fallback صامت)
MALFORMED_PROJECT_LINK_MESSAGE = (
    "⚠️ <b>رابط مشروع مشوّه.</b>\n"
    "الرابط يشبه رابط Genspark لكنه لا يحتوي معرّف مشروع (UUID) صالحاً.\n"
    "أرسل رابطاً كاملاً بصيغة <code>...agents?id=&lt;UUID&gt;</code> أو المعرّف الخام مباشرة."
)
...
def resolve_project_runtime_binding(
    ...
    *,
    ...
    return {
        ...
        # ⚡ [P43] العلم يُحسم هنا مرة واحدة عند بدء المهمة (الحدية 1) عبر
        # الدالة الحاكمة D3 — GitHub مفعّل يفوز دائماً ويعيد False.
        "fast_lean_skip": should_skip_artifacts_download(settings),
...
def apply_project_runtime_binding(
    ...
    *,
    ...
    if bridge_cfg is not None:
        ...
        # ⚡ [P43] fast_lean_skip يركب bridge_cfg بنفس نمط باقي الحقول —
        # يصل للـ worker بلا أي قراءة Registry إضافية وقت الاكتمال (الحدية 1).
        bridge_cfg.project_fast_lean_skip = bool(binding.get("fast_lean_skip", False))
    return binding
...
def should_skip_artifacts_download(project_settings: dict) -> bool:
    """⚡ [P43] الدالة الحاكمة للوضع السريع (D3 حرفياً — وسيط واحد).

    GitHub مفعّل ⇒ التنزيل إلزامي دائماً (يفوز على fast_mode).
    وإلا ⇒ يُتخطى التنزيل/فك الضغط/snapshot فقط عند fast_mode=True.
    الافتراضي False للجميع (D5/R2) — لا حقل ثانٍ ولا Invariant (R1).
    """
...
# ══════════════════════════════════════════════════════════════
SESSION_EXPIRED_KEYWORDS = ["session expired", "session منتهية", "login required", "not logged in"]
...
CREDIT_EXHAUSTED_KEYWORDS = [
    ...
    "you've used all your credits", "used all your credits", "credits are insufficient", "fromurl=credit_exhausted",
    "action_credit_exhausted", "__credit_exhausted__", "upgrade to continue", "out of credits", "genspark.ai/pricing",
    "pricing?fromurl=", "kindly visit this page to add more", "كريدت منتهية"
]
...
def detect_response_status(response: str | dict | None) -> str:
    ...
    # 🌐 [P45] كشف حالة انتهاء الجلسة من كود الاستجابة الفعلي للـ endpoint فقط (استجابة الدومين وليس نص الشات)
    if isinstance(response, dict):
        status_code = response.get("status") or response.get("code") or response.get("status_code") or response.get("error_code")
    ...
    if any(kw in check_text for kw in SESSION_EXPIRED_KEYWORDS):
        return "SESSION_EXPIRED"
    # 🛡️ [P46] كشف FORBIDDEN حصراً من كود الـ endpoint أعلاه وليس من نص الشات
    pass
...
def current_attempt_reply(messages, evidence=None):
    """Select within the latest user turn; never cross a pending /compact."""
    candidate = None
    scoped_user = False
    for message in reversed(messages or []):
        ...
        if message.get("role") == "user":
            if evidence:
                ...
                fresh_summary = (isinstance(state, dict) and state.get("is_compact_summary") is True
                                 and message.get("id") in evidence.get("summary_ids", ()))
                if message.get("id") != evidence.get("request_id") and not fresh_summary:
                    return None
                scoped_user = True
            break
        if candidate is None and message.get("role") == "assistant":
            candidate = message
    if candidate and evidence:
        if not scoped_user and candidate.get("id") not in evidence.get("message_ids", ()):
            return None
        if (not candidate.get("id") or candidate["id"] in evidence.get("old_message_ids", ())
                or (candidate.get("project_id") and candidate["project_id"] != evidence.get("project_id"))):
            return None
    return candidate
...
def resolve_runtime_credit_status(raw_status, response, mod, pid, cookies, cfg, message=None):
    ...
    if isinstance(evidence, dict) and evidence.get("project_id") == pid:
        if evidence.get("credit_exhausted") is True:
            return "CREDIT_EXHAUSTED"
        if message is None:
            message = evidence.get("reply")
        if (raw_status == "CREDIT_EXHAUSTED" and evidence.get("finished") is True
                and not has_platform_credit_signal(message)
                and (str(response) != "__CREDIT_EXHAUSTED__" or
                     (isinstance(message, dict) and message.get("content") == response))):
            return "COMPLETED"
...
def fetch_final_reply_text(mod, pid, cookies, cfg, old_text, email: str = ""):
    ...
    try:
        ...
        if isinstance(evidence, dict):
            last_asst = current_attempt_reply(msgs, evidence)
...
class ProjectRegistry:
    ...
    def _normalize_project_settings(self, settings: dict | None) -> dict:
        ...
        base["fast_mode"] = bool(payload.get("fast_mode", False))
    ...
    def update_project_settings(self, settings: dict | None = None) -> dict:
        with self.lock:
            ...
            if "fast_mode" in patch:
                # ⚡ [P43] حقل واحد bool حصراً (D5/R1) — أي قيمة تُطبع bool
                current["fast_mode"] = bool(patch.get("fast_mode", False))
    ...
    def set_compact_state(self, due, source_pid, duration=0.0, context=None, *,
                          deferred=False, chat_session_id="", bypass_ready=None, trigger_status=None):
        ...
        if trigger_status in ("CREDIT_EXHAUSTED", "COMPLETED"):
            ...
            state["due"] = (trigger_status == "CREDIT_EXHAUSTED" and due is True
                            and 0 <= state["duration_seconds"] <= COMPACT_TRIGGER_SECONDS)
        if context is not None:
            state.update({"chat_session_id": context["chat_session_id"],
                          "summary_key": context["summary_key"], "verified": bool(context["summary_key"])})
            if context.get("deferred"):
                state.update({"deferred": True, "bypass_ready": True, "due": False})
        with self.lock:
            data = self._read()
            ...
            if (context is None and trigger_status not in ("CREDIT_EXHAUSTED", "COMPLETED")
...
def render_project_status_text(project_key: str) -> str:
    ...
    model_line = f"\n• الموديل: <code>{html_escape(format_model_display_label(settings.get('model')))}</code>"
...
def render_project_settings_text(project_key: str) -> str:
    ...
    return (
        ...
        f"• الموديل: <code>{html_escape(format_model_display_label(settings.get('model')))}</code>\n"
...
def build_project_model_keyboard(*, back_callback: str = "cmd:show_dashboard", back_label: str = "⬅️ رجوع للوحة التحكم") -> dict:
    ...
    for model_name in AVAILABLE_MODELS:
        rows.append([{"text": MODEL_DISPLAY_NAMES.get(model_name, model_name), "callback_data": f"cmd:new_proj_model:{model_name}"}])
...
def format_copied_settings_summary(result: dict) -> str:
    ...
    return (
        ...
        f"<b>الموديل:</b> <code>{html_escape(format_model_display_label(settings.get('model')))}</code>\n"
...
def describe_terminal_outcome(status: str | None, pub_url: str | None, bridge_cfg: BridgeConfig | None = None) -> dict:
    ...
    mapping = {
        ...
        "ACTIVITY_STOPPED": (
            "<b>توقفت المتابعة لتغيّر مؤشر النشاط.</b>",
            "تم الالتزام بالتوقف الفوري دون إعلان اكتمال غير مثبت أو إرسال متداخل. راجع المشروع قبل الاستئناف.",
        ),
        "CREDIT_UNCONFIRMED": (
    ...
    if "callback_query" in update:
        ...
        if data == "cmd:show_dashboard":
            ...
            if state.get("action") == "AWAITING_NEW_PROJECT_MODEL":
                ...
                send_telegram_message(chat_id, f"✅ <b>الموديل المختار:</b> <code>{html_escape(format_model_display_label(selected_model))}</code>\nهل تريد تفعيل GitHub لهذا المشروع من البداية؟", reply_markup=build_new_project_github_choice_keyboard())
