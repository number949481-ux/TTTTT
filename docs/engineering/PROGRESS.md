...
| الحقل | القيمة |
|---|---|
| **last-updated** | 2026-08-22 — S59 (P38: توحيد عرض إيميل الحساب النشط عبر كل بطاقات دورة حياة المشروع — Unified Active Account Email) |
...
| **current WBS phase** | **P38 — توحيد عرض إيميل الحساب النشط عبر كل بطاقات دورة حياة المشروع (Unified Active Account Email)** 📧 ✅ مغلقة — بوابة 761/761 Exit 0 |
...
| **current-task** | `TSK-5403` (DONE) ➔ S59 مكتملة: **P38** داخل `01.33` (**7807 أسطر**، +15) — Unified Active Account Email (طلب مالك مباشر INSPECT-FIRST): الإيميل كان في بطاقة الاكتمال فقط والبارامتر `stage_email` مهمل ➔ دالة مركزية `format_active_account_line` (تفريغ آمن + fallback «غير محدد» + html_escape) محقونة في البطاقات الخمس: ⚡ اللايف الفوري + 🔁 handoff الرصيد (getattr الآمن من cfg) + 🔄 اللقطة (stage_email أولاً ثم cfg) + ✅/🚫 الاكتمال/الرفض (توحيد التسمية «📧 الحساب:» مع الحفاظ الحرفي على journey_block — عقد P29) + 🟢 اللايف المكتملة (used_acc) + **25 حارساً جدد** في `test_p38_unified_account_email_display.py` — الإجمالي **761** + مرايا p11/p12 محدثة + parity 11/11 + نظافة git المرة 25 (أُصلحت قبل أي تعديل — خط أساس أخضر أولاً). المرجع السابق: `TSK-5303` (DONE) — P37 Decline ➔ Resume Summary Card — 29 حارساً 
| **next-action** | اختبار تشغيلي حي (E2E) من المالك: **P38** (بدء مهمة ➔ سطر 📧 الحساب يظهر في بطاقة اللايف فوراً وفي بطاقات اللقطة/handoff/الاكتمال بنفس الصياغة الموحدة) + **P37** (برومبت يُرفض ➔ ضغط ✍️ أعد صياغة البرومبت ➔ بطاقة 🔄 ملخص الاستئناف تفتح فوراً ➔ ▶️ كمل الآن + برومبت جديد يكمل على نفس المشروع) + **P36** (برومبت يُتوقع رفضه عبر المحرك ➔ رسالة الرفض تصل لحظياً بلا انتظار ensure_public/share/download) + **P35** (برومبت يُتوقع رفضه ➔ رسالة 🚫 بالزرين الملونين + استئناف بعدها يكمل من النقطة الصالحة قبل الرفض) + **P34** (مهمة برد طويل >2500 حرف ومتعددة الحسابات ➔ الرسالة الختامية تنزل كاملة بكل أزرارها) + **P33** (بناء مهمة للاكتمال ➔ ضغط ▶️ كمل الآن = استئناف فوري + ضغط ⬅️ رجوع = لوحة كاملة) + P32 (زر 🔐 ➔ بحث هجين) + P29/P30 (مهمة متعددة الحسابات) + P28 (ملف .txt فعلي) + تفويض GitHub لدفع Push/PR يدوياً (المزامنة التلقائية تدفع لـ main) |
...
| **completion** | 761/761 Tests Verified (100%) 🧪 |
| **quality-gate** | `python scripts/hadith_sijil.py` ➔ 761/761 PASS — Exit Code 0 |
| **session-log** | `docs/engineering/SESSION_LOG.md` |
| **release decision** | `READY` 🟢 (P38: توحيد عرض إيميل الحساب النشط — سطر 📧 الحساب الموحد في كل بطاقات دورة الحياة (لايف/handoff/لقطة/اكتمال/رفض) بدالة مركزية واحدة + P37: فتح بطاقة ملخص الاستئناف من زر إعادة الصياغة — ضغطة واحدة = بطاقة الملخص فوراً + P36: مسار الرفض السريع في المحرك — استجابة الرفض لحظية + P35: كشف رفض الموديل والتعافي — الرفض كأن الطلب لم يُرسل + P34: التنسيق الآمن وحدود الأحرف — Total Message Guard + P33: أزرار الإجراءات السريعة برسالة الاكتمال + P32: استخراج باسورد الحساب ببحث هجين + P29: مسار رحلة الحسابات الحي + P30: توقيت كل حساب Unmasked في الرسالة النهائية + P31: صفر نداء لكوين عند sync بلا تغييرات + P28: استقبال ملفات المهام .txt/.md + P27: تصفح المشاريع 20/صفحة + P26: حذف ذري آمن + P25: إلغاء تفاعلي + P24: كوميت ذكي بكوين + P20/P21: REST-Only + DATA_RETENTION + P18: وقف فوري لمؤشر النشاط — صفر انحدار) |
