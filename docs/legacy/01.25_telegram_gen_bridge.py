...
"""
╔══════════════════════════════════════════════════════════════╗
║          🌐 ⚡ 01.25_telegram_gen_bridge.py                 ║
║  إصدار المعاينة الحية الفورية وحارس تجربة المستخدم (P7-A):   ║
...
║  • كتم إشعارات الأرشيفات الفنية الزائدة D-002                ║
...
BUILD_VERSION = "01.25"
BUILD_PARENT_BASELINE = "01.24"
BUILD_PARENT_BASELINE_SHA256 = "0124_frozen_golden_baseline"
...
def detect_response_status(response: str | dict | None) -> str:
    """
    كشف واستخراج حالة الرد بدقة بين SESSION_EXPIRED, FORBIDDEN, CREDIT_EXHAUSTED, RUNNING, COMPLETED.
...
def _safe_extract_archive(archive_bytes: bytes, out_dir: pathlib.Path) -> bool:
    """غلاف توافق قديم فوق diagnostic extractor."""
    return bool(_extract_archive_with_diagnostics(archive_bytes, out_dir).get("ok"))
...
def send_message_and_make_public(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if not cookies_valid and (time.time() - last_refresh) > 120:
        ...
        for chat_attempt in range(2):
            try:
                ...
                if on_project_start_callback is not None:
                    send_chat_kwargs["on_project_start_callback"] = on_project_start_callback
        ...
        while final_status not in ("COMPLETED", "CREDIT_EXHAUSTED", "SESSION_EXPIRED", "FORBIDDEN"):
            ...
            try:
                if hasattr(mod, "fetch_project_messages"):
                    ...
                    if latest_msgs:
                        last_c = latest_msgs[-1].get("content", "")
                        if last_c:
...
class ProjectRegistry:
    ...
    def _git_native_sync_uploader(self, payload: dict, token: str, git_exe: str) -> dict:
        """مزامنة ورفع الملفات إلى GitHub باستخدام Git Native (Clone & Push) المعزول لكل مشروع،
        مستوحاة من 04_upload_to_Fable_github.py مع عزل كامل لمنع أي تعارض."""
        import tempfile
        repository = payload["repository"]
        branch = payload["branch"]
        target_root_rel = str(payload.get("target_root") or "/").strip("/")
        upload_files = payload.get("upload_files", [])
        delete_files = payload.get("delete_files", [])
        skipped = list(payload.get("skipped", []))

        temp_dir = tempfile.mkdtemp(prefix=f"git_sync_{self.key}_")
        clone_dir = pathlib.Path(temp_dir) / "repo"
        auth_url = f"https://x-access-token:{token}@github.com/{repository}.git"

        try:
            # 1) Clone shallow
            clone_cmd = [git_exe, "clone", "--depth", "1", "-b", branch, auth_url, str(clone_dir)]
            clone_res = subprocess.run(clone_cmd, capture_output=True, text=True)
            if clone_res.returncode != 0:
                # لو الفرع غير موجود أو الريبو جديد، نجرب clone عادي أو init
                init_res = subprocess.run([git_exe, "clone", "--depth", "1", auth_url, str(clone_dir)], capture_output=True, text=True)
                if init_res.returncode != 0:
                    raw_err = clone_res.stderr or init_res.stderr or ""
                    clean_err = redact_github_secrets(str(raw_err).strip())
                    raise RuntimeError(f"GIT_CLONE_FAILED: {clean_err}")
                # Switch/Create branch

                subprocess.run([git_exe, "checkout", "-B", branch], cwd=str(clone_dir), capture_output=True, text=True)

            # 2) ضبط هوية الـ Git محلياً
            subprocess.run([git_exe, "config", "user.name", "AI Bridge Bot"], cwd=str(clone_dir), capture_output=True)
            subprocess.run([git_exe, "config", "user.email", "bot@genspark.ai"], cwd=str(clone_dir), capture_output=True)
            subprocess.run([git_exe, "config", "core.quotepath", "false"], cwd=str(clone_dir), capture_output=True)

            # 3) نسخ وتحديث الملفات النشطة
            active_rel_paths = set()
            for file_info in upload_files:
                src_path = pathlib.Path(file_info["local_path"])
                rel = self._normalize_remote_relative_path(file_info["path"])
                if _should_skip_archive_member(rel) or _is_never_copy_file(pathlib.Path(rel).name):
                    skipped.append(rel)
                    continue
            ...
            # 4) الحذف المرآتي الذكي (Smart Mirror Delete) مع حماية الملفات والمجلدات الخاصة
            protected_prefixes = {".git", ".github", ".agents", ".vscode"}
            protected_files = {".gitignore", "README.md", "LICENSE"}

            for root_path, dirs, files_in_dir in os.walk(str(dest_root)):
                rel_root = os.path.relpath(root_path, str(dest_root)).replace("\\", "/")
                # تجنب مسح المجلدات المحمية
                dirs[:] = [d for d in dirs if d not in protected_prefixes and f"{rel_root}/{d}".strip("/") not in protected_prefixes]

                for f_name in files_in_dir:
                    rel_file = f_name if rel_root == "." else f"{rel_root}/{f_name}"
                    rel_file = pathlib.PurePosixPath(rel_file).as_posix()

                    # الحماية الشرطية
                    if any(rel_file.startswith(f"{p}/") or rel_file == p for p in protected_prefixes):
                        continue
                    if rel_file in protected_files and rel_file not in active_rel_paths:
                        continue

                    if rel_file not in active_rel_paths:
                        target_del = dest_root / rel_file
                        if target_del.exists() and target_del.is_file():
                            target_del.unlink(missing_ok=True)

            # 5) فحص التغييرات عبر git status
            status_res = subprocess.run([git_exe, "status", "--porcelain"], cwd=str(clone_dir), capture_output=True, text=True)
            if not status_res.stdout.strip():
                return {"uploaded": [], "modified": [], "deleted": [], "unchanged": [f["path"] for f in upload_files], "skipped": skipped, "commit_hash": ""}

            # 5) Stage & Diff
            subprocess.run([git_exe, "add", "-A"], cwd=str(clone_dir), check=True, capture_output=True)
            diff_res = subprocess.run([git_exe, "diff", "--cached", "--name-status"], cwd=str(clone_dir), capture_output=True, text=True)

            new_files, updated_files, deleted_files = [], [], []
            for line in (diff_res.stdout or "").splitlines():
                if not line.strip():
                    continue
                parts = line.split("\t")
                if len(parts) >= 2:
            ...
            # 6) Commit & Push
            summary_msg = f"[{self.key}] update: {len(new_files)} added, {len(updated_files)} modified, {len(deleted_files)} deleted"
            subprocess.run([git_exe, "commit", "-m", summary_msg], cwd=str(clone_dir), check=True, capture_output=True)
            push_res = subprocess.run([git_exe, "push", "origin", branch], cwd=str(clone_dir), capture_output=True, text=True)
            if push_res.returncode != 0:
                raise RuntimeError(f"GIT_PUSH_FAILED: {push_res.stderr}")

            commit_hash_res = subprocess.run([git_exe, "rev-parse", "HEAD"], cwd=str(clone_dir), capture_output=True, text=True)
            commit_hash = (commit_hash_res.stdout or "").strip()[:8]

            return {
                "uploaded": new_files,
                "modified": updated_files,
    ...
    def _default_github_uploader(self, payload: dict) -> dict:
        ...
        # 1) محاولة استخدام Git Native أولاً
        git_exe = shutil.which("git")
        if git_exe:
            try:
                return self._git_native_sync_uploader(payload, token, git_exe)
            except Exception as git_err:
                clean_err = redact_github_secrets(str(git_err))
                log_event("warning", f"فشل Git Native Sync ({clean_err})؛ جاري التحويل إلى REST API fallback...")


        # 2) Fallback إلى GitHub Contents REST API
        import base64
        import requests
        headers = build_github_api_headers(token)
        uploaded, unchanged, deleted, skipped = [], [], [], list(payload.get("skipped", []))
        ...
        return {"uploaded": uploaded, "unchanged": unchanged, "deleted": deleted, "skipped": skipped}
...
def is_project_run_claimed(project_key: str) -> bool:
    key = re.sub(r"[^A-Za-z0-9_-]", "_", str(project_key or ""))[:80]
    with PROJECT_RUN_OWNERS_GUARD:
        return key in PROJECT_RUN_OWNERS
...
def restore_registry_index_from_backup() -> bool:
    backup = _registry_index_backup_path()
    if not backup.exists() or not backup.is_file():
        return False
    try:
    ...
    if not isinstance(raw, dict):
        return False
    schema_version = raw.get("schema_version")
    if schema_version not in (None, REGISTRY_INDEX_SCHEMA_VERSION):
        return False
    normalized = _normalize_registry_index_payload(raw)
    temp = PROJECT_REGISTRY_INDEX_FILE.with_suffix(".restore.tmp")
    temp.write_text(json.dumps(normalized, ensure_ascii=False, indent=2), encoding="utf-8")
    temp.replace(PROJECT_REGISTRY_INDEX_FILE)
    return True
...
def build_project_picker_keyboard(chat_id: int, limit: int = 6) -> dict:
    rows = []
    for record in list_known_projects(chat_id=chat_id, limit=limit):
        snap = get_project_dashboard_snapshot(record["project_key"])
        label = f"📁 {snap['project_name'][:20]}"
        rows.append([
            {"text": label, "callback_data": f"proj:{snap['project_key']}"},
            {"text": "⭐ التفاصيل", "callback_data": f"pview:{snap['project_key']}"},
        ])
    if not rows:
        rows.append([{"text": "🚀 مشروع جديد", "callback_data": "cmd:new_proj"}])
    rows.append([{"text": "⬅️ رجوع للدashboard", "callback_data": "cmd:show_dashboard"}])
...
def build_project_settings_keyboard(project_key: str) -> dict:
    ...
    return make_inline_keyboard([
        ...
        [{"text": "⬅️ رجوع للدdashboard", "callback_data": "cmd:show_dashboard"}],
    ])


def build_project_resume_summary_keyboard(project_key: str, *, target_url: str = "", target_pid: str = "") -> dict:
    rows = [
        [{"text": "▶️ كمل الآن", "callback_data": "cmd:resume_continue"}, {"text": "⚙️ عدّل الإعدادات", "callback_data": "cmd:resume_settings"}],
    ]
    if target_url:
        rows.append([{"text": "🌐 فتح المشروع", "url": target_url}])
    if target_pid:
        rows.append([{"text": "🌳 نقاط الاستئناف", "callback_data": f"tree:{target_pid}"}])
    rows.append([{"text": "📁 مشاريعي", "callback_data": "cmd:list_projects"}, {"text": "⬅️ رجوع للدdashboard", "callback_data": "cmd:show_dashboard"}])
    return make_inline_keyboard(rows)
...
def build_bound_project_branch_choice_keyboard() -> dict:
    return build_project_branch_choice_keyboard(
        ...
        back_label="⬅️ رجوع للدdashboard",
...
def build_current_project_keyboard(project_key: str) -> dict:
    ...
    rows.append([
        ...
        {"text": "❌ إلغاء", "callback_data": f"pctl:cancel:{project_key}"},
    ])
...
def build_dashboard_keyboard(chat_id: int) -> dict:
    rows = [
        [{"text": "🚀 مشروع جديد", "callback_data": "cmd:new_proj"}, {"text": "📁 مشاريعي", "callback_data": "cmd:list_projects"}],
...
def build_project_model_keyboard(*, back_callback: str = "cmd:show_dashboard", back_label: str = "⬅️ رجوع للدdashboard") -> dict:
    rows = []
...
def build_new_project_model_keyboard(*, back_callback: str = "cmd:show_dashboard", back_label: str = "⬅️ رجوع للدdashboard") -> dict:
