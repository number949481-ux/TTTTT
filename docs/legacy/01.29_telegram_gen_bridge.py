...
"""
╔══════════════════════════════════════════════════════════════╗
║          🌐 ⚡ 01.29_telegram_gen_bridge.py                 ║
║  إصدار الصلابة التشغيلية (P12+P13+P14+P16+P17+P18):        ║
...
BUILD_VERSION = "01.29"
BUILD_PARENT_BASELINE = "01.28"
BUILD_PARENT_BASELINE_SHA256 = "0128_p17_hardening_baseline"
...
def _generate_ai_commit_message(git_exe: str, clone_dir: str, new_files, updated_files, deleted_files) -> str | None:
    """🤖 [P17] رسالة كوميت ذكية عبر qwen_engine (نفس محرك 04_upload).

    العقد:
    - يبني changed_files_lines + short_diff من الـ staged changes الفعلية.
    - يستدعي qwen_engine.generate_ai_summary ويرجع سطر COMMIT فقط.
    - أي فشل (استيراد/شبكة/حسابات/رد فارغ) → None بصمت والرسالة الثابتة تبقى fallback.
    - مهلة زمنية ضمنية عبر عقد qwen_engine نفسه (سباق حسابات 30 ثانية).
    """
    try:
        import qwen_engine  # مستقل بجوار السكربت — لا يُحمَّل إلا عند الحاجة
    except Exception:
        return None
    try:
        changed_lines = (
            [f"A\t{p}" for p in (new_files or [])]
            + [f"M\t{p}" for p in (updated_files or [])]
            + [f"D\t{p}" for p in (deleted_files or [])]
        )
        if not changed_lines:
            return None
        diff_res = subprocess.run(
            [git_exe, "diff", "--cached", "--unified=1"],
            cwd=clone_dir, capture_output=True, text=True, timeout=30,
        )
        short_diff = (diff_res.stdout or "")[:6000]
        commit_msg, _summary, _model = qwen_engine.generate_ai_summary(changed_lines, short_diff, [])
        if commit_msg and isinstance(commit_msg, str):
            commit_msg = commit_msg.strip().splitlines()[0][:120]
            return commit_msg or None
    except Exception:
        pass
    return None
...
class ProjectRegistry:
    ...
    def _git_native_sync_uploader(self, payload: dict, token: str, git_exe: str) -> dict:
        ...
        try:
            ...
            # 6) Commit & Push
            # 🤖 [P17] رسالة كوميت ذكية عبر qwen_engine (نفس محرك 04_upload) مع fallback آمن للرسالة الثابتة
            summary_msg = f"[{self.key}] update: {len(new_files)} added, {len(updated_files)} modified, {len(deleted_files)} deleted"
            ai_commit = _generate_ai_commit_message(git_exe, str(clone_dir), new_files, updated_files, deleted_files)
            if ai_commit:
                summary_msg = f"[{self.key}] {ai_commit}"
