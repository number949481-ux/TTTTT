...
> **الكتالوج الشامل:** يوثق الـ 164 فحصاً التاريخية التراكمية للمشروع + الـ **761 فحصاً النشطة محلياً** لإصدار `01.33` + المحرك `01.03`  
> **حالة الاختبارات النشطة:** 761/761 PASS (100% OK) — بوابة `hadith_sijil.py` بـ Exit Code 0 ⚡  

## 📊 0. المصفوفة النشطة الحالية — 01.33 + المحرك 01.03 (761 فحصاً / 32 ملفاً)
...
| **P38 توحيد عرض إيميل الحساب النشط عبر كل بطاقات دورة حياة المشروع (Unified Active Account Email)** | `test_p38_unified_account_email_display.py` | **25** | **دالة مركزية `format_active_account_line(raw_email)` (مصدر واحد للحقيقة: تفريغ/تقليم آمن ➔ fallback «غير محدد» ➔ `html_escape` ➔ `📧 <b>الحساب:</b> <code>{email}</code>\n` — لا ترمي أبداً مهما كان المدخل) + الحقن في البطاقات الخمس: ⚡ اللايف الفوري (getattr الآمن من cfg + موقع السطر بين PID والموديل + منع الوصول المباشر cfg.attr) + 🔁 handoff الرصيد + 🔄 اللقطة/المزامنة (تفعيل البارامتر المهمل `stage_email` أولاً ثم fallback لـ cfg) + ✅/🚫 الاكتمال/الرفض (التسمية الموحدة «📧 الحساب:» + زوال «الحساب المستعمل» + الحفاظ الحرفي على `{journey_block}` عقد P29 + منع التهريب المزدوج لـ acc_email + الرفض مغطى بنفس res_msg) + 🟢 اللايف المكتملة P7-A (used_acc) + التوحيد الشامل (عدد الحقن ≥4 + سطر «الحساب النشط» P29 بلا مساس + تصدير المرايا p11 + لا ترمي أبداً)** |
| **الإجمالي** | **32 ملفاً** | **761** | ✅ 761/761 PASS |
