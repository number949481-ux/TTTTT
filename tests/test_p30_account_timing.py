...
class TestFinalMessageBlock(unittest.TestCase):
    ...
    def test_single_account_block_always_shown(self):
        """SCENARIO A: الكتلة تظهر حتى بحساب واحد (عكس سطر P29 الشرطي)."""
    ...
    def test_finisher_is_last_span_account(self):
        """SCENARIO B: «(المُنجِز)» لآخر حساب في الرحلة فقط."""
        cfg = self._cfg_with_spans([self._mk("a@x.com", 10), self._mk("b@x.com", 20)])
        block = _bridge.format_account_timing_block(cfg)
        a_line = [l for l in block.split("\n") if "a@x.com" in l][0]
        b_line = [l for l in block.split("\n") if "b@x.com" in l][0]
        self.assertNotIn("المُنجِز", a_line)
        self.assertIn("المُنجِز", b_line)

    def test_returning_account_finisher_and_multiplier(self):
        """SCENARIO C: A→B→A — A هو المُنجِز ويحمل ×2."""
        cfg = self._cfg_with_spans([self._mk("a@x.com", 10), self._mk("b@x.com", 20), self._mk("a@x.com", 5)])
        block = _bridge.format_account_timing_block(cfg)
        a_line = [l for l in block.split("\n") if "a@x.com" in l][0]
        self.assertIn("المُنجِز", a_line)
    ...
    def test_accounts_total_is_sum_of_spans(self):
        ...
        block = _bridge.format_account_timing_block(cfg)
        self.assertIn("زمن تشغيل الحسابات", block)
        self.assertIn("2 دقيقة", block)

    def test_resume_counter_independent_of_accounts_count(self):
        """SCENARIO H: Resume ≠ Accounts−1 — 3 حسابات مع استئناف واحد فقط."""
        cfg = self._cfg_with_spans(
            [self._mk("a@x.com", 1), self._mk("b@x.com", 1), self._mk("c@x.com", 1)],
            continuations=1,
        )
        block = _bridge.format_account_timing_block(cfg)
        self.assertIn("1 / 10", block)
        self.assertNotIn("2 / 10", block)
