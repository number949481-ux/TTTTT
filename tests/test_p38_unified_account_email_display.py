...
class TestP38CompletionAndDeclineCard(unittest.TestCase):
    def test_01_unified_label_in_res_msg(self):
        self.assertIn('f"📧 <b>الحساب:</b> <code>{acc_email}</code>{journey_block}\\n"', BRIDGE_SRC)
    ...
    def test_03_journey_block_contract_preserved(self):
        # عقد P29 الحرفي: journey_block ملاصق لسطر الحساب — بلا مساس
        self.assertIn("</code>{journey_block}", BRIDGE_SRC)
