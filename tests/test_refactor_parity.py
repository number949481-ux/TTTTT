...
والملف المرجعي 01.33_telegram_gen_bridge.py:
...
class TestBehaviorSpotChecks(unittest.TestCase):
    def test_detect_response_status(self):
        ...
        self.assertEqual(ns.detect_response_status({"code": 401}), "SESSION_EXPIRED")
        ...
        # [P45] نصوص الشات التي تحتوي على 401 أو unauthorized لا تسبب SESSION_EXPIRED كاذب
        self.assertEqual(
            ns.detect_response_status("التوكن الذي أرسلته مرفوض مرة أخرى: git push --dry-run → Authentication failed، REST → 401. القراءة العامة تعمل."),
            "COMPLETED",
        )
        self.assertEqual(
            ns.detect_response_status("def handle_api_error(err):\n    if '401' in err or 'unauthorized' in err:\n        return False"),
            "COMPLETED",
        )
        self.assertEqual(ns.detect_response_status("Session expired: please log in again"), "SESSION_EXPIRED")
        # [P46] كشف FORBIDDEN من كود الاستجابة الفعلي للـ endpoint وحصانة نصوص الشات
        self.assertEqual(ns.detect_response_status({"status": 403}), "FORBIDDEN")
        self.assertEqual(ns.detect_response_status({"code": 403}), "FORBIDDEN")
        self.assertEqual(
            ns.detect_response_status("def handle_api_error(err):\n    if '403' in err or 'forbidden' in err:\n        return False"),
            "COMPLETED",
        )
        self.assertEqual(
            ns.detect_response_status("HTTP 403 Forbidden - Access denied / permission denied for user"),
            "COMPLETED",
        )
