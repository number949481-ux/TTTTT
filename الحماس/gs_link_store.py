...
def _is_valid_pid(pid: str) -> bool:
    """التحقق من صحة معرّف المشروع (UUID v4 مكون من 36 حرفاً) بعد التنظيف والتحويل للـ lowercase"""
    if not pid or not isinstance(pid, str):
        return False
    return bool(_UUID_RE.match(pid.strip().lower()))
...
def get_pids(key: str = "default") -> list:
    ...
    if not isinstance(raw_list, list):
        return []
    return [p.strip().lower() for p in raw_list if _is_valid_pid(p)][:MAX_KEEP]
...
def push_pid(pid: str, key: str = "default", owner: str | None = None) -> bool:
    ...
    with _LOCK:
        data = _read()
        existing = [p.strip().lower() for p in data.get(key, []) if _is_valid_pid(p)]
