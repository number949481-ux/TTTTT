...
# 📦 تبعية صلبة معتمدة: مخزن معرّفات مشاريع الشات (آخر 3 PIDs)
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
if _SCRIPT_DIR not in sys.path:
    sys.path.insert(0, _SCRIPT_DIR)
import gs_link_store
...
class Config:
    ...
    # ── ملف الحسابات ──
    accounts_file: str = "Genspark_V5.5/accounts_genspark_V5.5.json"
...
def send_chat(
    ...
    if cfg.model in ("gpt-5.5", "claude-opus-4-8"):
        ...
        # ── inject use_model and client_message_id for claude-fable-5 ──
        if cfg.model == "claude-fable-5":
            payload["use_model"] = "claude-fable-5"
            payload["client_message_id"] = user_msg_id
    ...
    try:
        ...
        r = sess.post(f"{GENSPARK}/api/agent/ask_proxy", json=payload, timeout=cfg.timeout)
        if cfg.show_debug:
        ...
        if r.status_code not in (200, 204):
            ...
            if r.status_code == 500 and project_id:
                ...
                return None, "__INVALID_PROJECT__", None
            p(Fore.RED, f"  ❌ Error {r.status_code}: {r.text[:80]}")
            return None, None, None


        # ── تحليل SSE + طباعة لحظية ──
        full_text = ""
        proj_id_new = project_id
        asst_msg_id = str(uuid.uuid4())
        proj_name = ""
        live_started = False  # عشان نطبع الـ header مرة واحدة
        _t_start = time.time()
        print(f"\r  ⏳ بيفكر...", end="", flush=True)  # مؤشر انتظار
        ...
        for line in r.text.splitlines():
            ...
            try:
                ...
                # streaming delta — هنا بيطبع لحظي!
                if t == "message_field_delta" and obj.get("field_name") == "content":
                    chunk = obj.get("delta", "")
                    full_text += chunk
                    # ── طباعة لحظية في Terminal ──
                    if not live_started:
                        elapsed = time.time() - _t_start
                        print(f"\r  ⏳ رد بعد {elapsed:.1f}s", end="")
                        print()
        ...
        # اطبع الرد — لو مكنش طباعة لحظية (non-streaming)
        if not live_started:
            print()
            p(Fore.GREEN + Style.BRIGHT, "  ┌─ الرد ─────────────────────────────────────────┐")
            ...
            print()  # سطر جديد بعد الطباعة اللحظية
...
def _do_ask_parallel_worker(model_dict: dict, email: str, cookies: dict, query: str, cfg: Config, accounts: list, skip_emails: set):
    ...
    if USE_URL_MODE:
        ...
        if _last and _last.get("project_id"):
            # ✅ JSON فيه بيانات → كمّل من آخر محادثة (entry_url بيتجاهل)
            _start_project_id = _last.get("project_id")
            if cfg.entry_url:
                p(Fore.CYAN, f"  📌 Continue Mode: من آخر JSON ({_start_project_id[:16]}...)")
        elif cfg.entry_url:
            # 🔀 JSON فاضي → Fork من entry_url كـ seed أول مرة بس
            p(Fore.CYAN, f"  🌱 Seed Mode: Fork أول مرة من entry_url...")
            _fork_history = fork_from_url(cfg.entry_url, cookies)
            _start_project_id = None
